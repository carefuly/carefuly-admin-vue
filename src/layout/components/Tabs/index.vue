...
<template>
  <el-tabs v-model="activeTab" type="card" class="sky-tabs" @tab-remove="removeTab" @tab-click="clickToggleTab">
    ...
    <el-tab-pane v-for="item in tabList" :key="item.path" :label="item.title" :name="item.path" :closable="item.closeIcon">
      ...
      <template #label>
        <div class="flex justify-center flex-items-center" @contextmenu.prevent="handleTabsMenuParent(item.path, $event)">
          <SkyGlobalIcon class="m-r-2px" v-show="item.icon" :name="item.icon" size="16"></SkyGlobalIcon>
          <div class="p-t-1px">
            <div>{{ item.title }}</div>
          </div>
        </div>
