...
// 点击面包屑
const handleBreadcrumb = (item: any, index: any) => {
  if (index !== breadcrumbList.value.length - 1) router.push(item.redirect);
};
</script>
    ...
    <el-breadcrumb :separator-icon="ArrowRight">
      <transition-group name="breadcrumb">
        <el-breadcrumb-item v-for="(item, index) in breadcrumbList" :key="item.path">
          ...
          >
            <SkyGlobalIcon class="breadcrumb-icon" v-if="item.meta.icon" :name="item.meta.icon" size="16"></SkyGlobalIcon>
