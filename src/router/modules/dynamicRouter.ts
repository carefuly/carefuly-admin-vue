...
import {getShowStaticAndDynamicMenuList, getAllBreadcrumbList} from "@/utils";
import {generateRoutes, generateFlattenRoutes} from "@/utils/filterRoute";
// import {listRoute} from "@/apis/sky/system/menu";
...
export const initDynamicRouter = async () => {
  ...
  try {
    ...
    //   generateRoutes(getShowStaticAndDynamicMenuList(res.data), null)
    // );
    // // 面包屑需要静态和动态所有的数据，无论是否隐藏
    // authStore.breadcrumbList = staticRouter.concat(generateRoutes(res.data, null));
    // // 2. 按钮权限列表 && 递归菜单数据
